 ...
 * request/response, over one socket. Deliberately separate from
 * `network:fetch`, which is one-shot and allowed-domain-scoped.
 *
 * The channel *families* a plugin may touch are not expressed here — see
 * `Capability`'s doc comment. Room-level scoping is enforced broker-side.
 */
